    ...
    static _handleRequestError(error) {
        ...
        if (error.response) {
            ...
            if (error.response.data.collection) {
                errMsg = cj_1.default.getErrorMessage(error.response.data.collection);
            }
            apiError = new exception_1.default(errMsg);
            ...
            apiError = new exception_1.default('No server response!');
            apiError.request = error.request;
        }
        else {
            // Something happened in setting up the request that triggered an Error
            //console.log('Error', error.message);
            apiError = new exception_1.default(error.message);
        }
        throw apiError;
