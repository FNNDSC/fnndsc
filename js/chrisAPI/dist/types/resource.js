...
exports.ListResource = exports.ItemResource = exports.Resource = void 0;
/** * Imports ***/
...
class ItemResource extends Resource {
     ...
     */
    get(timeout = 30000) {
        const req = new request_1.default(this.auth, this.contentType, timeout);
    ...
    get data() {
        ...
        return cj_1.default.getItemDescriptors(this.collection.items[0]);
    }
    /**
    ...
    getPUTParameters() {
        if (this.collection && this.collection.template) {
            return cj_1.default.getTemplateDescriptorNames(this.collection.template);
        }
        return null;
    }
    /**
    ...
    _getResource(linkRelation, ResourceClass, searchParams = null, timeout = 30000) {
        ...
        const urls = cj_1.default.getLinkRelationUrls(item, linkRelation);
        if (!urls.length) {
            const errMsg = 'Missing "' + linkRelation + '" link relation!';
            throw new exception_1.default(errMsg);
        }
    ...
    _put(data, uploadFileObj, timeout = 30000) {
        ...
        if (!uploadFileObj && this.contentType === 'application/vnd.collection+json') {
            putData = { template: cj_1.default.makeTemplate(data) };
        }
...
class ListResource extends Resource {
    ...
    getSearchParameters() {
        if (this.collection) {
            if (this.collection.queries) {
                const params = cj_1.default.getQueryParameters(this.collection.queries);
    ...
    getItem(id) {
        ...
        const items = this.collection.items.filter((item) => cj_1.default.getItemDescriptors(item).id === id);
        if (!items.length) {
            return null;
        }
        const itemResource = new this.itemClass(items[0].href, this.auth);
        itemResource.collection = cj_1.default.createCollectionObj();
    ...
    get data() {
        ...
        for (let item of this.collection.items) {
            data.push(cj_1.default.getItemDescriptors(item));
        }
        return data;
    }
    /**
    ...
    get totalCount() {
        if (this.collection) {
            return cj_1.default.getTotalNumberOfItems(this.collection);
        }
        return -1;
    }
    /**
    ...
    get hasNextPage() {
        if (this.collection) {
            const urls = cj_1.default.getLinkRelationUrls(this.collection, 'next');
            if (urls.length) {
                return true;
            }
        }
    ...
    get hasPreviousPage() {
        if (this.collection) {
            const urls = cj_1.default.getLinkRelationUrls(this.collection, 'previous');
            if (urls.length) {
                return true;
            }
        }
    ...
    _getResource(linkRelation, ResourceClass, searchParams = null, timeout = 30000) {
        if (!this.collection) {
            throw new exception_1.default('Collection object has not been set!');
        }
        const urls = cj_1.default.getLinkRelationUrls(this.collection, linkRelation);
        if (!urls.length) {
            const errMsg = 'Missing "' + linkRelation + '" link relation!';
            throw new exception_1.default(errMsg);
        }
    ...
    _post(data, uploadFileObj, timeout = 30000) {
        ...
        let postData = data;
        if (!uploadFileObj && this.contentType === 'application/vnd.collection+json') {
            postData = { template: cj_1.default.makeTemplate(data) };
        }
