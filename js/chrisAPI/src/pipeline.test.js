    ...
    it('can delete a pipeline resource through a REST API DELETE request', done => {
      ...
      const data = {
        ...
        plugin_tree: JSON.stringify([
          { plugin_id: dsappPluginId, title: 'P1', previous: null },
        ]),
      };
    ...
    before(() => {
      ...
      return pipeline
        ...
        .then(pipings => {
          ...
          expect(pipingItems.length).to.be.at.least(2,
      ...
      return new Promise((resolve, reject) => {
        Request.runAsyncTask(function*() {
          try {
            ...
            if (!items.length) {
              throw new Error(
                'no pipeline source files found — re-run pre_test.sh to seed one'
              );
            }
            sourceFile = items[0];
          } catch (ex) {
